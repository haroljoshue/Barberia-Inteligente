...
namespace Barberia.MVC.Controllers
{
    ...
    public class ClienteController : Controller
    {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            var cliente =
                await client.GetFromJsonAsync<ApplicationUser>(
                    $"api/usuarios/{clienteId}");

            var citas =
                await client.GetFromJsonAsync<List<Cita>>(
                    $"api/citas/cliente/{clienteId}") ?? new();
            ...
            var vm = new ClienteDashboardViewModel
            {
                ...
                CitasPendientes =
                    citas.Count(c =>
                        c.Estado == ModelosBarberia.Enum.EstadoCita.Pendiente),

                CitasCompletadas =
                    citas.Count(c =>
                        c.Estado == ModelosBarberia.Enum.EstadoCita.Atendida),
        ...
        public async Task<IActionResult> Agendar()
        {
            ...
            var barberos =
                await client.GetFromJsonAsync<List<Barbero>>(
                    "api/barberos") ?? new();

            var servicios =
                await client.GetFromJsonAsync<List<Servicio>>(
                    "api/servicios") ?? new();
        ...
        public async Task<IActionResult> Agendar(AgendarCitaRequest request)
        {
            ...
            var response =
                await client.PostAsJsonAsync("api/citas", request);
